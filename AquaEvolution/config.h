...
static constexpr int32_t DUMP_FREQ = INT32_MAX;
...
constexpr uint64_t FISH_START = 500;
constexpr uint64_t ALGAE_START = 5000;
...
// Alage parameters
constexpr uint64_t ALGAE_MAX_COUNT = 50000;
...
// Fish parameters
constexpr uint64_t FISH_MAX_COUNT = 15000;
constexpr float FISH_MAX_ENERGY = 50.0f;
constexpr float FISH_INITAL_ENERGY = 30.0f;
constexpr float FISH_ENERGY_PER_KID = 20.0f;
...
inline void initMutationsFromConfig(Mutation& mutation) {
	...
	mutation.host.sightAlteration[4] = make_float2(1.005f, -0.005f); // (dist -> multiplier, angle -> addition)
	...
	mutation.host.sightAlteration[5] = make_float2(0.995f, 0.005f); // (dist -> multiplier, angle -> addition)
